...
const STATS: { value: string; label: string; borderClass: string; shadowClass: string; valueClass: string }[] = [
  ...
  { value: "100+", label: "EXPERIMENTS RAN", borderClass: "border-poke-blue", shadowClass: "shadow-[5px_5px_0_#2a75bb]", valueClass: "text-[#7fbaff]" },
  { value: "10+", label: "INTERFACES BUILT", borderClass: "border-fire-red", shadowClass: "shadow-[5px_5px_0_#e3350d]", valueClass: "text-[#ff8a7a]" },
];

export default function Origin() {
  return (
    <section id="origin" className="relative overflow-hidden border-y-[5px] border-ink bg-ink">
      <CornerSprite src="/sprites/gyarados-pixel-e9dd4a8b.png" corner="top" side="right" size={120} durationMs={3400} />
      <CornerSprite src="/sprites/pikachu-pixel-2.png" corner="bottom" side="left" size={104} />

      <div className="mx-auto max-w-[1160px] px-[clamp(16px,4vw,40px)] py-[clamp(56px,9vw,84px)]">
        <Reveal>
        ...
        <div className="mt-[46px] grid grid-cols-1 gap-[22px] sm:grid-cols-3">
          {STEPS.map((step, i) => (
            <Reveal
              key={step.number}
              delay={i * 80}
            ...
            >
              ...
              <p className="mt-3 text-[17px] leading-[1.7] text-dark-text">{step.body}</p>
            </Reveal>
          ))}
        </div>

        <Reveal as="p" className="mt-11 text-[19px] leading-[1.8] text-dark-text-muted text-pretty">
        ...
        <div className="mt-11 grid grid-cols-1 gap-[22px] sm:grid-cols-3">
          {STATS.map((stat, i) => (
            <Reveal
              key={stat.label}
              delay={i * 80}
              className={`rounded-2xl border-4 bg-dark-card px-[26px] py-[26px] text-center ${stat.borderClass} ${stat.shadowClass}`}
            >
              <div className={`font-heading text-[30px] ${stat.valueClass}`}>{stat.value}</div>
              <div className="mt-3 font-pixel text-[12px] text-dark-text">{stat.label}</div>
            </Reveal>
          ))}
        </div>
      </div>
